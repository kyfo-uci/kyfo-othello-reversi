...
/* join_room_respomse:
    {
        ...
        'socket_id': the soket of the user that just joined the room
    }
or
    {
        'result': 'fail',
 ...
 */
    ...
    socket.on('join_room',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('join_room command failure',JSON.stringify(response));
            return;
        }
        ...
        if ((typeof room == 'undefined') || (room === null)){
            response = {
                result: 'fail',
                message: 'client did not send room to join'
            }
            socket.emit('join_room_response',response);
            serverLog('join_room command failure',JSON.stringify(response));
            return;
        ...
        io.in(room).fetchSockets().then((sockets)=>{
            ...
            /* Socket did join the room */
                ...
                /* Announce to everyone who else is in the room */
    ...
    socket.on('invite',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('invite command failure',JSON.stringify(response));
            return;
        }
    ...
    socket.on('uninvite',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('uninvite command failure',JSON.stringify(response));
            return;
        }
        let requested_user = payload.requested_user;
        let room = players[socket.id].room;
    ...
    socket.on('game_start',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('game_start command failure',JSON.stringify(response));
            return;
        }
        let requested_user = payload.requested_user;
        let room = players[socket.id].room;
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")){
            response = {
                result: 'fail',
                message: 'the user that was engaged in play is not in a room'
            }
            socket.emit('game_start_response',response);
            serverLog('game_start command failure',JSON.stringify(response));
            return;
    ...
    */
    ...
    socket.on('send_chat_message',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('send_chat_message failure',JSON.stringify(response));
            return;
        }
        let room = payload.room;
        let username = payload.username;
        ...
        if ((typeof room == 'undefined') || (room === null)){
            response = {
                result: 'fail',
                message: 'client did not send room to message'
            }
            socket.emit('send_chat_message',response);
            serverLog('send_chat_message failure',JSON.stringify(response));
            return;
        ...
        if ((typeof username == 'undefined') || (username === null)){
            response = {
                result: 'fail',
                message: 'client did not send a valid username as a message sender'
            }
            socket.emit('send_chat_message',response);
            serverLog('send_chat_message command failure',JSON.stringify(response));
            return;
        }
    ...
    socket.on('play_token',(payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)){
            ...
            serverLog('play_token command failure',JSON.stringify(response));
            return;
        }
        ...
        if ((typeof player == 'undefined') || (player === null)){
            response = {
                result: 'fail',
                message: 'play_token came from an unregistered user'
            }
            socket.emit('play_token_response',response);
            serverLog('play_token command failure',JSON.stringify(response));
            return;
...
function create_new_game(){
    ...
    /* Reversi rules: black goes first */
...
/* Retund true if r + dr supports playing at r and c + dc support playing at c */
function adjacent_support(who,dr,dc,r,c,board) {
    ...
    if (who === 'b'){
        ...
        log("That errs in ignorance and not in cunning:"+who);
        return false;
    }
